...
userRouter.post("/signupusername", async (req, res) => {
  ...
  try {
    ...
    console.log(error);
    res.status(202).json({
      error: "backend is down will fix asap",
    });
  }
});
...
userRouter.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!token) {
      res.redirect(`${FRONTEND_URL}/erro`);
    } else {
...
userRouter.get('/fetchIds', auth, async (req: CustomRequest, res) => {
  const userId = req.user_id;

  try {
    if (!userId) throw new Error("No user ID found");

    let response = await pgClient.workspaces.findFirst({
    ...
    if (!response) {
      response = await pgClient.workspaces.create({
        data: {
          name: "general",
          owner_id: userId,
          channels: {
    ...
    res.status(200).json({
      ...
      channelId: response.channels[0]?.id || null, // Ensure safety
    });
